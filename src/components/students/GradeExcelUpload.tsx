...
function downloadTemplate(studentName: string) {
  ...
  XLSX.writeFile(wb, `درجات_${studentName}.xlsx`);
}

function validateRow(row: any[]): GradeRow {
  const errors: string[] = [];
  const week = Number(row[0]) || 0;
  const exam = Number(row[1]) || 0;
  const homework = Number(row[2]) || 0;
  const participation = Number(row[3]) || 0;
  const classInteraction = Number(row[4]) || 0;
  const project = Number(row[5]) || 0;
  const practical = Number(row[6]) || 0;

  if (week < 1) errors.push("رقم الأسبوع غير صحيح");
  if (exam < 0 || exam > MAX_SCORES.exam) errors.push(`الاختبار يجب أن يكون بين 0 و ${MAX_SCORES.exam}`);
  if (homework < 0 || homework > MAX_SCORES.homework) errors.push(`الواجبات يجب أن تكون بين 0 و ${MAX_SCORES.homework}`);
  if (participation < 0 || participation > MAX_SCORES.participation) errors.push(`المشاركة يجب أن تكون بين 0 و ${MAX_SCORES.participation}`);
  if (classInteraction < 0 || classInteraction > MAX_SCORES.class_interaction) errors.push(`التفاعل يجب أن يكون بين 0 و ${MAX_SCORES.class_interaction}`);
  if (project < 0 || project > MAX_SCORES.project) errors.push(`المشروع يجب أن يكون بين 0 و ${MAX_SCORES.project}`);
  if (practical < 0 || practical > MAX_SCORES.practical) errors.push(`العملي يجب أن يكون بين 0 و ${MAX_SCORES.practical}`);
...
export function GradeExcelUpload({ studentId, studentName }: Props) {
  const [open, setOpen] = useState(false);
  const [parsedRows, setParsedRows] = useState<GradeRow[]>([]);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success(`تم رفع ${validRows.length} درجة بنجاح`);
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setParsedRows([]); setStep("upload"); } }}>
      ...
      <DialogContent className="max-w-2xl" dir="rtl">
        <DialogHeader>
          <DialogTitle>رفع درجات من ملف إكسل</DialogTitle>
        </DialogHeader>
        ...
        {step === "upload" && (
          <div className="space-y-4 mt-3">
            <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center space-y-4">
              ...
              <div>
                <p className="text-sm text-muted-foreground mb-2">
                  حمّل القالب أولاً، عبّئ الدرجات، ثم ارفعه هنا
                </p>
            ...
            <div className="bg-muted/50 rounded-lg p-3 text-xs text-muted-foreground space-y-1">
              ...
              <p>• يمكنك إضافة عدة أسابيع في ملف واحد</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4 mt-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleUpload} disabled={uploading || validCount === 0}>
                {uploading ? "جارٍ الرفع..." : `رفع ${validCount} درجة`}
              </Button>
            </div>
          </div>
        )}
